...
class MaskOptimizer:
    ...
    def new_get_m_opt(model, unmasked_size):
        ...
        #input_img = np.ones(shape=model.layers[0].input_shape[1:])[None, :] / 2  # define an initial random image
        ###chat gpt since im not 100% sure what is it supposed to hold
        batch_shape = model.layers[0].batch_shape # This includes the batch size as the first element

        # Create the array with the same shape, but set batch size to 1
        array_shape = (1,) + batch_shape[1:]
        input_img = np.ones(shape=array_shape) / 2
        ###
        grad, loss = MaskOptimizer.gradient(model, input_img)
        grad = np.negative(np.squeeze(grad))  # change sign
        m_opt = MaskOptimizer.new_get_mask_from_grads(grad, unmasked_size, model.layers[0].output.shape[1])
        return m_opt
    ...
    def new_check_loss_for_opposite_indexes(model, m_opt, min_index, max_index, opposite_indexes):
        m_opt_changed = False
        m_opt_loss = model.predict(m_opt[None, :], verbose=0)
        for ind in opposite_indexes:
            m_new_opt = np.copy(m_opt)
            m_new_opt[max_index] = 1
            m_new_opt[ind] = 0
            m_new_opt_loss = model.predict(m_new_opt[None, :], verbose=0)
    ...
    def new_check_for_likely_change(model, m_opt, min_index, max_index, m_opt_grad):
        m_opt_changed = False
        m_opt_loss = np.squeeze(model.predict(m_opt[None, :], verbose=0))
        ...
        m_new_opt[max_index] = 1
        m_new_opt_loss = np.squeeze(model.predict(m_new_opt[None, :], verbose=0)) # i add , verbose=0 want to avoid from print loss time calc
