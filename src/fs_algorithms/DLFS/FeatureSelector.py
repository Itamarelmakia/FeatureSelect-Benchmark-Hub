...
# Initialize TensorFlow to recognize the change
tf.get_logger().setLevel('ERROR')  # Suppresses most messages


def mean_squared_error(y_true, y_pred):
    return tf.reduce_mean((y_true - y_pred) * (y_true - y_pred), axis=1)
...
class FeatureSelector():
 ...
 #               self.operator.model.set_weights(self.operator.ES_best_weights)
  #              break
            if (self.operator.useEarlyStopping is True and self.operator.ES_stop_training is True):
                if self.operator.ES_best_weights is not None: # fix the typo of : TypeError: object of type 'NoneType' has no len()
                    self.operator.model.set_weights(self.operator.ES_best_weights)
                    break
